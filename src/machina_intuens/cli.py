...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Machina-Intuens thinks for you about the meanings.",
        epilog="Example:  machina-intuens input_text.txt > output_text.txt"
...
def run():
    """
    $ text | machina-intuens                        # Accepts text from the pipe
    $ echo "...<text>..." | machina-intuens         #

    $ machina-intuens multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="machina-intuens",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"machina-intuens: {utterance}.")
        syslog.closelog()
